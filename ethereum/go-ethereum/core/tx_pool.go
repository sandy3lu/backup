...
type TxPool struct {
	...
	CaFile  string // add by LR for CA
}

// NewTxPool creates a new transaction pool to gather, sort and filter inbound
// transactions from the network.
func NewTxPool(config TxPoolConfig, chainconfig *params.ChainConfig, chain blockChain, caFile string) *TxPool {
...
func (pool *TxPool) promoteTx(addr common.Address, hash common.Hash, tx *types.Transaction) {
	...
	log.Debug("TxPool.promoteTx: verify CA ")
	//verify Transaction by CA
	err:=tx.VerifyCA(pool.CaFile)
	if err!=nil{
		log.Error("Failed verify TX to chain", "err", err)
		return
	}
...
func (pool *TxPool) addTx(tx *types.Transaction, local bool) error {
	pool.mu.Lock()
	defer pool.mu.Unlock()
	log.Debug("TxPool.addTx: ",tx.Hash(),local)
